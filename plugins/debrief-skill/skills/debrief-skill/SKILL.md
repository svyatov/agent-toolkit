...
The `<version>` in a cache path is the version that ran, and `version` in the source plugin's `.claude-plugin/plugin.json` is the current one. When the source has moved past it, a friction the current text already fixes is not a finding. A skill that loads straight from its source (a user skill, or a symlink into a checkout) has no version: write `(loads from source)` in the header, and check `git status` there. An uncommitted edit is what ran when the file changed before the run's Step 1 timestamp. A file changed after it (compare `stat -f %m`, or `stat -c %Y` on Linux) ran as `git show HEAD:<path>`, so debrief the run against that text. For a symlink, `readlink <dir>` names the checkout.
